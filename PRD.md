...
| --- | --- | --- | --- |
| User preview and demonstration experience | Completed | General accounts may log in and prepare tenant-scoped drafts before verification while final submission retains the server-side verification gate; the author dashboard summarizes detailed workflow states as Waiting, Reviewed, Evaluation, and Accepted; login, registration, and the database-backed homepage use the responsive ANG editorial design; the idempotent seed produced one explicitly fictional annual volume with three issues and ten articles per issue in local and production databases; authentication/submission integration tests, typecheck, production build, five-route axe checks, three visual baselines, all seven E2E journeys, READY API/web production deployments, and production API/content smokes pass with exactly 3 issues and 30 articles | None |
