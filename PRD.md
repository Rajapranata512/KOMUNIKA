...
| --- | --- | --- | --- |
| User preview and demonstration experience | In progress | General accounts may log in and prepare tenant-scoped drafts before verification while final submission retains the server-side verification gate; the author dashboard summarizes detailed workflow states as Waiting, Reviewed, Evaluation, and Accepted; registration and the database-backed homepage use the responsive ANG editorial design; an idempotent local seed produced one fictional annual volume with three issues and ten articles per issue; authentication/submission integration tests, typecheck, production build, five-route axe checks, three visual baselines, all seven E2E journeys, READY API/web production deployments, and read-only production health/content smokes pass | Production seed execution is blocked pending explicit approval for the separate 30-article production data mutation |
